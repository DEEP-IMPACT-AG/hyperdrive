...
func (b *Box) Draw(screen tcell.Screen) {
	...
	if b.border && b.width >= 2 && b.height >= 2 {
		...
		if b.title != "" && b.width >= 4 {
			_, printed := tview.Print(screen, b.title, b.x+1, b.y, b.width-2, b.titleAlign, b.titleColor)
			if tview.StringWidth(b.title)-printed > 0 && printed > 0 {
				_, _, style, _ := screen.GetContent(b.x+b.width-2, b.y)
				fg, _, _ := style.Decompose()
				tview.Print(screen, string(tview.SemigraphicsHorizontalEllipsis), b.x+b.width-2, b.y, 1, tview.AlignLeft, fg)
			}
		}
	}
...
func (b *Box) GetFocusable() tview.Focusable {
	return b.focus
}
